...
const AddPayment = ({ paymentId, createPayment, updatePayment, onClose }) => {
  ...
  const getUnpaidTask = useCallback(async (clientId) => {
    try {
      ...
      setUnpaidTask(result.tasks.map((item) => { return { value: item.id, label: item.name, date: Date(item.date_pay), price: `${item.residue.price.toFixed(2)} ${item.residue.currency}` } }));
    } catch (e) {
      console.log(e);
    }
  }, [])
